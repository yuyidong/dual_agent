...
def main() -> None:
    ...
    parser.add_argument("--data", default=None, help="Dataset generated by scripts/data/generate_dataset.py.")
    parser.add_argument("--forecaster-checkpoint", default=None)
    parser.add_argument("--surrogate-checkpoint", default="checkpoints/surrogate.pt")
    parser.add_argument("--checkpoint", default="checkpoints/dual_agent.pt")
    parser.add_argument(
        "--phase3-checkpoint-dir",
        default="checkpoints/phase3_states",
        help="Directory for per-stage phase-3 forecaster/surrogate checkpoints.",
    )
    ...
    phase3_checkpoint_dir = Path(args.phase3_checkpoint_dir)
    phase3_checkpoint_dir.mkdir(parents=True, exist_ok=True)

    def save_forecaster_state(stage: int) -> None:
        torch.save(
            cpu_state_dict(system.forecaster),
            phase3_checkpoint_dir / f"forecaster_{stage:02d}.pt",
        )

    def save_surrogate_state(stage: int) -> None:
        torch.save(
            cpu_state_dict(system.surrogate),
            phase3_checkpoint_dir / f"surrogate_{stage:02d}.pt",
        )

    save_forecaster_state(0)
    save_surrogate_state(0)
    print(f"phase3_checkpoint_dir={phase3_checkpoint_dir.resolve()}")
    print("saved phase3 state: F0/S0")
    ...
    forecaster_optimizer = torch.optim.AdamW(
        system.forecaster.parameters(), lr=config.training.phase3_forecaster_learning_rate
    )
    surrogate_optimizer = torch.optim.AdamW(
        system.surrogate.parameters(), lr=config.training.phase3_surrogate_learning_rate
    )
    ...
    # Select checkpoints by the composite dispatch objective used for training.
    ...
    try:
        for round_index, forecaster_epochs, surrogate_epochs in schedule:
            ...
            for surrogate_epoch in range(surrogate_epochs):
                ...
                if validation_cost < best_pair_score:
                    best_pair_score = validation_cost
                    best_pair_state = copy.deepcopy(system.state_dict())
            ...
            for round_epoch in range(forecaster_epochs):
                ...
                print(
                    ...
                    f"epoch={round_epoch + 1:03d} train_loss={train_loss:.6f} "
